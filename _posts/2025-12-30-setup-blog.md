...
GitHub Pages와 Jekyll을 이용하여 정적 블로그를 구축하였습니다. 이제 테마 선정부터 Chirpy 테마 설정, 그리고 Giscus 댓글 연동까지의 과정을 글로 작성해보겠습니다.
...
초기에는 `Simplex` 등의 테마를 고려했으나, 빌드 의존성 문제와 커스텀 플러그인 호환성 이슈 등 초보인 저한테는 어려웠습니다. 그래서 최종적으로 개발자 블로그로서 기능이 충실하고 설정이 간편한 **[Chirpy](https://github.com/cotes2020/chirpy-starter)** 테마를 선택했습니다.
...
이 방식을 사용하면 GitHub Actions가 내장되어 있어, 글을 작성하고 Push만 하면 자동으로 빌드 및 배포가 이루어져, 간단한 설정이면 바로 자신의 블로그 형태가 보여지게 됩니다.
...
lang: ko
timezone: Asia/Seoul
title: "블로그 이름" # 저는 swh Log로 작성했습니다.
tagline: "주식과 코딩을 기록하는 공간" # 특수문자가 포함될 경우 반드시 따옴표("") 사용
url: "[https://swh00.github.io](https://swh00.github.io)"
github:
  username: swh00
'''
...
### 카테고리 및 태그 사용
Chirpy 테마는 별도의 설정 없이 포스트의 머리말에 선언하는 것만으로 카테고리 페이지가 자동 생성되어, 관리하기 편하다는 생각이 들었습니다.
...
정적 블로그에서 댓글 기능을 구현하기 위해 **Giscus**를 도입했습니다. Giscus는 GitHub Discussions를 기반으로 작동하며, 광고가 없고 GitHub 계정과 연동된다는 장점이 있습니다.
...
## 5. 포스트 작성 가이드
그렇게 해서 Chirpy를 통해 완성된 블로그는 아래와 같이 생겼습니다.

Chirpy 테마에서 글을 작성하는 표준 절차는 다음과 같습니다.

### 파일 생성 규칙
모든 포스트는 `_posts` 디렉토리 안에 생성해야 하며, 파일명은 반드시 **`YYYY-MM-DD-제목.md`** 형식을 따라야 합니다. 그렇지 않고 날짜 형식이 틀리면 사이트에 노출되지 않는다.

* **올바른 예:** `2025-01-01-hello-world.md`
* **잘못된 예:** `hello-world.md` (날짜 누락), `2025-1-1-title.md` (자릿수 오류)
/assets/img/2025/img_path.png
...
### 1. 이미지 업로드
블로그 루트 디렉토리의 `assets/img/` 폴더에 이미지를 업로드한다. (관리를 위해 날짜별 폴더를 만들거나 `posts` 폴더를 만들어 관리하는 것을 권장합니다.)

* 경로 예시: `assets/img/2024/chart_01.png`

### 2. 마크다운에 이미지 삽입
글 작성 시 절대 경로(`/`)를 사용하여 이미지를 불러온다.

```markdown
![이미지 설명](/assets/img/2024/chart_01.png)
```

**Tip:** 캡션을 달거나 사이즈를 조절하고 싶다면 HTML 태그를 직접 사용하거나 테마에서 제공하는 확장 문법을 사용할 수 있다.
...
앞으로 이것저것 올릴 수 있는 개인 블로그가 생겼으니, 뭘 작성할진 차차 고민하겠습니다.