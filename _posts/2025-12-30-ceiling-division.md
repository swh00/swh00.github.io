---
layout: post
title: "모듈 없이 올림(Ceiling) 구현하기 (정수 연산 팁)"
...
보통은 `math` 모듈을 import 해서 `math.ceil()`을 사용하지만, 라이브러리 호출 없이 **정수 연산(Integer Arithmetic)**만으로 훨씬 간결하고 빠르게 처리하는 방법이 있습니다.
...
## 2. 일반적인 해결법 (math 모듈)
가장 정석적인 방법은 `math` 모듈을 사용하는 것이 있습니다.
...
* **편의성:** `import`가 필요 없다.
* **활용:** 페이징 처리, 그리드 계산 등에서 필수적으로 사용되는 테크닉이다.
