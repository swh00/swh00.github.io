...
마침 깃허브 사용을 익숙하게 할겸, **Chrome 확장 프로그램**과 **GitHub Actions**를 조합하여 저만의 자동화 시스템을 구축해보았습니다.

---

## 📅 Phase 1. 시작: 백준허브(BaekjoonHub) 도입
처음에는 단순히 "푼 코드를 깃허브에 올리고 싶다"는 생각으로 시작했습니다.

### 🛠️ 설치 및 연동
Chrome 확장 프로그램인 **[BaekjoonHub]**를 설치했습니다.
- **장점:** 백준에서 '제출' 버튼만 누르면 자동으로 내 GitHub 리포지토리에 코드가 푸시(Push)된다는 점이었습니다.
...
아무래도 markdown과 프로그램 언어를 이용하여 스크립트를 작성해본 경험이 적어서, AI의 도움을 받아 스크립트를 작성했습니다.
AI를 사용하면서 항상 느끼는 점은, 사용자에 따라 AI성능도 달라지는 것 같습니다. 왜냐하면 저는 한번에 에러없는 코드를 얻어본적이 없기 때문이죠.
하지만 몇번 코드에서 이상한점을 찾고, 오류를 AI한테 설명해주는 과정을 거치면 간단한 코드들은 바로 실행가능하게 되니, 참 매력적인건 변하지 않는 것 같습니다.

아무튼 이제 백준에서 문제를 풀면, 해당 문제는 깃허브에 저장되고, 자동으로 풀이 노트가 Issue에 생성됩니다.
이슈가 자동으로 생성되지 않으면, Actions탭에서 
...
혹시 필요한 분들을 위해 최종 완성된 `.github/workflows/auto-issue-link.yml` 코드와, 해당 코드에 필요한 파이썬 스크립트 `scripts/issue_linker.py`를 공유합니다.
...
🚀 결론
단순 반복 작업을 자동화하니 문제 풀이와 회고에만 집중할 수 있게 되었다. 특히 Shields.io 뱃지로 티어가 색깔별로 딱 박히는 걸 보면 수집욕이 생겨서 더 열심히 풀게 되는 것 같다.

앞으로 GitHub 잔디 심기와 함께 알고리즘 티어도 쑥쑥 오르길! 🌱
